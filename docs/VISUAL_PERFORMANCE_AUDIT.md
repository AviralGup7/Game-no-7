...
Campaign topology/content: OK {"reachable_walkable_cells": 13440, "perimeter_edges": 1676}
```
...
Worst-case per-frame triangles with three districts resident (`max_visible_sectors = 3`, 105 m cull): floors are `3 × 224 = 672` ground instances × `808` tris ≈ **543 k**, plus a few hundred connector-deck modules near the player; perimeter rails are the swing factor — the station merges to **180 wall AABBs / 13 408 m**, rendered as one `MultiMeshInstance3D` per 8 m segment (**1 676 instances** of the 22 442-tri `wall.glb`), of which only the frustum-visible subset draws (a full district edge is 480 m = 60 segments ≈ 1.35 M tris, a typical 720p frustum sees ~150 – 250 m ≈ **0.4 – 0.7 M**); enemies add `≤ 18 × ~300 ≈ 5 k` and the Warden hero is one 34 860-tri skinned mesh (GPU skinning, not batch count). Typical frame ≈ **1.0 – 1.3 M tris**, worst case (three full district rails on screen) ≈ **2.0 M** — at or above the ~1.5 M tris/frame comfort budget for a 2019-class Adreno at 720p. This is the single largest remaining performance risk on the restored twelve-district station and is tracked in the backlog below (batch rails per merged AABB instead of per 8 m segment, and cull them with their district); it is **not** a gate failure, because draw calls stay low (`MultiMesh` batching) and nothing exceeds a validated budget.

Per SCENE totals: campaign floors are not 3 768 `MeshInstance3D`s — they are 47 `MultiMeshInstance3D`s. Arena.tscn inlines 43 `MeshInstance3D`s for editor triplanar inspection; see §9.
...
**Rule:** Campaign `floors >64` and `props >80` warn, tscn `MeshInstance3D >80` warn / `>150` error. Floor *modules* are judged against the runtime budget instead of a flat count: `FLOOR_MODULE_BUDGET = ArenaNavGrid.WORLD_CELL_LIMIT / NAV_CELL² = 40 960 / 4 = 10 240` modules (warn at 7 680), **and** the campaign must actually batch them — if `campaign_geometry.gd` has no `MultiMesh` the module count is an error whatever it is.

| Source | Count | Gate | Result |
|--------|-------|------|--------|
| Floors (`station_zero.json`) | **47** rects | <64 | ✔ |
| Props (campaign cover) | **72** | <80 | ✔ |
| Districts | **12** (4 × 3 mesh) | resident = `max_visible 3 × 224` modules = **672** ≤ 1 200 | ✔ |
| Floor modules (8 m) | **3 768** modules (216 × 168 nav grid, 864 × 672 m footprint) | ≤ 10 240 budget, `MultiMesh`-batched | ✔ |
...
The campaign would be an object-count disaster if `CampaignGeometry.floor_batch` built per-module `MeshInstance3D`s (3 768 draw calls plus 3 768 nodes). It does not — it builds one `MultiMesh` per floor rect. The arena still does per-mesh — see §9.
...
**Finding:** Collision complexity is minimal and correct. Every solid is a box, broadphase-friendly, and the merged perimeter prevents 3 768 floor-tile colliders.
...
| `CampaignGeometry.floor_batch` | 0 | **47** (one per `floors` rect) | 4 styles (military/hazard/tech/rusted) | **47** (not 3 768) | <80 | ✔ Batched |
| `CampaignGeometry.wall_batch` | 0 | merged perimeter AABBs → `count = round(length/8)` MultiMesh segments | 4 styles | merged, not per-edge (1 676 authored edges) | <80 | ✔ Batched |

The campaign inlines zero per-module meshes. `floor_batch` does one `MultiMesh` per `floors` rect (47), each instance-count equals the module count for that rect — 3 768 modules share 47 `MultiMesh` objects. `wall_batch` splits each merged perimeter AABB at `MODULE=8` boundaries so district theme changes happen at the right place without sealing an authored causeway.
...
**Rule:** `floors >64` error (fragmentation streaming cost); `max_visible_sectors >3` warns (keep three districts resident); **resident module budget** `max_visible_sectors × mean modules per district ≤ RESIDENT_MODULE_BUDGET = 1 200` — the real streaming cost is what stays in memory, not how many districts exist; `station_zero.tscn` must remain <10 KB thin wrapper (so the heavy world streams via code, not as an inlined 1000-node scene); large `.tscn >600 KB` warns for load time.

| Fact | Value | Gate | Finding |
|------|-------|------|---------|
| Campaign floors | **47** rects (12 districts `128 × 112`, 17 connectors `48 × 16` / `16 × 48`, 8 ring spurs `16 × 72`, 6 side spurs `40 × 16`, 4 spines `736 × 16` / `16 × 608`) | <64 | ✔ |
| Districts | **12** (4 × 3 mesh + perimeter service ring) | 3 × 224 = 672 resident modules | ✔ ≤ 1 200 |
...
| Deck handling | `ServiceCauseways` / ring decks are distance-culled with the same 105 m radius as districts (at 6 × the earlier station size an always-resident deck set would dominate the frame); their **collision stays merged and always loaded**, so culling a deck never opens a hole in the nav mesh | intentional | ✔ |
...
|---|-------------|----------------|-----------------------------------|
| 1 | **Perimeter rails are one `MultiMeshInstance3D` per 8 m segment** — 13 408 m of merged wall becomes **1 676 single-instance batches**, and `PerimeterRails` is never distance-culled (only districts and connector decks are) | Frustum culling keeps the *drawn* subset small (~150 – 250 m on screen ≈ 0.4 – 0.7 M tris), but node count, AABB culling and worst-case tris all scale with station size — 5.5 × the six-district layout | Have `_render_perimeter_wall` group consecutive 8 m segments by wall style and hand each run to `wall_batch` (which already builds a real N-instance `MultiMesh`), then parent the runs to the district/deck they enclose so `update_visibility` culls them. ~1 676 nodes → ~50, and the worst-case frame drops by roughly a third. No art change, no gameplay change. |
| 2 | **Wall module is heavy** (22 442 tris, 7 602 KB `wall.glb`) | Batched via `MultiMesh`; the frustum-visible subset is 0.4 – 0.7 M tris (see row 1) — under budget on a typical frame, but the most tris per frame in the worst case | Generate a wall **LOD1** decimate (~6k tris, `VisibilityRange` 25–40 m) for far perimeter segments. Keeps the Mobile 60 fps headroom at 120 Hz tiers without new art. Cost: one import variant. |
| 3 | **Arena still inlines 43 meshes** | 43 draw calls <80 but not batched; wall Trim/Cornice could be MultiMesh | When Pit FPS is profiled on device, migrate `Wall_* / Trim_* / Cornice_*` to one `MultiMesh` per style — reuses the campaign batching path. Win is ~20 draw calls back. |
| 4 | **2048 wall textures** are the heaviest single textures | `Wall_normal` 4 234 KB / `Wall_albedo` 2 535 KB fit, but three styles × 16 MB VRAM each lives even when the style is not visible | Keep the distinct `Wall_albedo/normal` per style (they look right), but import with `COMPRESS Basis Universal` + `mipmaps` (already the project-wide default for new imports) and verify the 2048 mip tail is stripped on Mobile. Or downsize `Wall_*` `ORM/emission` (currently near-empty 1–12 KB) to 1024. |
| 5 | **No baked lightmap / GI** | Zero probes is correct for Mobile streaming, but the Pit's large stone/curtain area uses sky ambient only | If the Pit looks flat in a 1K HDR, add a single `LightmapGI` bake offline (Pit only, not campaign) — not `VoxelGI` — and keep `DirectionalLight` realtime. Campaign keeps sky ambient. |
| 6 | **PCF soft shadows at 2048** on Mobile | Budget gate is 2048 (pass) but 2048×2048 shadow atlas is the only atlas over 1K | Keep 2048 for 720p; if thermal throttling is observed, tie shadow atlas to the performance governor (`LOW = 1024/vars shadows off`, as already in `PerformanceGovernor`) and confirm 60 fps cap holds. |
| 7 | **Hero skinning 34 860 tris** | One skinned hero (single skeleton), not a batch; OK for ship | Add a 15k-tri LOD on the warden for the far minimap/ spectator camera. No gameplay change. |
| 8 | **Showcase renders bloat working tree** | 10 MB `ui_page_render.png`, 1–2 MB `*_render.png` previews not shipped | `.gdignore` the `chrome/` + `**/raw_*` capture folder or move them to `docs/` renders — not a runtime fix, just checkout discipline. |
...
1. **Derived module budgets.** `excessive_object_count` no longer warns at a flat `modules > 1200`. It computes `FLOOR_MODULE_BUDGET = ArenaNavGrid.WORLD_CELL_LIMIT / NAV_CELL² = 40 960 / 4 = 10 240` (warn at 7 680) — the largest floor union the runtime nav grid can hold — and **requires** `MultiMesh` batching in `campaign_geometry.gd`; unbatched, the same count is an error. The station sits at 3 768 modules.
2. **Resident-module streaming rule.** `streaming_scene_organization` replaced the flat `sectors > 8` warning (which merely punished a bigger station) with the quantity that actually costs frame time: `max_visible_sectors × mean modules per district ≤ RESIDENT_MODULE_BUDGET = 1 200`. Currently `3 × 224 = 672`.
3. **Honest worst-case arithmetic.** The per-frame triangle estimate is recomputed from the restored geometry (672 resident floor modules, 1 676 rail segments, 18 concurrent enemies) instead of the six-district figures, and the perimeter-rail batching cost is now the top backlog item.
4. **Data-derived verbose summary.** Every `--verbose` line is generated from the parsed map/scenes, so the report cannot drift from the data it describes.
