...
- Tests: 1348 python + the headless Godot suites (was 95). The audit-cleanup pass grew the base's
  1319 by 29 (`test_regress_audit_cleanup.py`: the constants home, the EventBus and fallback
  ledgers, and the gates that read them) and re-derived this count and
  `docs/campaign/README.md`'s executed count in the same commit, rather than leaving either stale.
  The two branches merged in
...
- Validated files: 183/183 (was 163, then 175: the eight authored skill cast-focus prop scenes)
