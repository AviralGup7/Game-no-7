...
2. **No GameRoot arena-selection command.** Existing `ContentRegistry.select_arena()` is not called by UI. Arena catalogue browsing is read-only; only the currently selected arena can launch. `UiCommands.select_arena()` can delegate to `GameRoot.request_arena_selection(id) -> bool` when the owning branch supplies it. That command must validate availability and update the arena selection used by the next run.
3. **No starter-weapon selection command.** Standard runs accurately advertise Gladius; daily runs advertise the DailyChallenge weapon. Other weapons are explicitly marked catalogue/owned previews, not falsely equipped. The owning branch must provide an authoritative starter-loadout API before those previews can become selectable.
...
5. **Save schema lacks FPS, remapping and touch-scale fields.** FPS and keyboard/gamepad rebindings are visibly session-only; there is no invented settings file or SaveManager fork. Touch layout adapts automatically. Persistent settings use only existing SettingsData setters and SaveManager commands. If the save-owning branch adds fields/APIs, wire them here rather than storing another copy.
...
- Covers boot/menu/setup, unsupported arena selection, command-launched standard/daily runs, pause/settings/back/resume, settings staging and rebind cancellation/conflict, touch capture cancellation, upgrade validation and double taps, HUD event updates, boss/reduced-motion, bounded effects, summary/reward idempotence, Armory purchases and persistence.
