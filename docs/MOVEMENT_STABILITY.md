...
| 9 | `scripts/arena/arena_hazards.gd` | per-frame hazard ticks dereferenced the arena-owned marker / disc / material with no validity check (a world rebuild frees them while the record still ticks), and a non-finite epicentre fed knockback | `HazardInstance.visual()` resolves the marker (null ⇒ skip the glow only); the vent still damages, and a non-finite centre is skipped rather than applied. The rebuild kept both guards and moved them onto typed state — the visual is a `HazardMarker` node now, not a `get_meta("disc")` round trip through a Dictionary record |
