...
## Dynamic FOV – base * mode multiplier + speed boost + combat boost, smoothed.
...
func tick(delta: float, velocity_tracker: CameraVelocityTracker, framing: CameraFramingController, mode: CameraModeController, camera: Camera3D, reduced_motion: bool) -> void:
	...
	# Mode multiplier is already blended (explore↔combat↔boss↔locked).
	if mode != null:
		target_fov = base_fov * mode.get_mode_fov_multiplier()

	# Speed boost (additive degrees on top of the mode-scaled base).
