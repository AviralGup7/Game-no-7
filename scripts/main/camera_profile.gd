...
##
## PUBG-style third-person shooter: over-the-shoulder, look-driven aim point,
## no Souls auto-follow. Sphere-cast boom and pitch limits stay.

@export var profile_id: StringName = &"default"

# --- Core framing (legacy kept) ---
@export var distance: float = 3.6
@export var height: float = 0.55
@export var field_of_view: float = 70.0
@export var follow_smoothing: float = 14.0
@export var look_height: float = 1.45
...
@export var yaw_smoothing: float = 18.0
@export var pitch_smoothing: float = 18.0
@export var fov_smoothing: float = 8.0
@export var position_smoothing: float = 16.0
@export var distance_smoothing_in: float = 18.0
@export var distance_smoothing_out: float = 10.0
@export var min_pitch_deg: float = -55.0
@export var max_pitch_deg: float = 70.0
@export var min_distance: float = 1.4
@export var max_distance: float = 6.5
@export var orbit_speed_deg: float = 180.0
...
@export var shoulder_offset: Vector2 = Vector2(0.85, 0.12)
@export var predictive_factor: float = 0.04
@export var look_ahead_distance: float = 0.15
@export var velocity_influence: float = 0.08
## World metres ahead of the shoulder the lens looks at (PUBG crosshair).
@export var aim_distance: float = 22.0
...
@export_group("Auto Follow (off for shooter look)")
...
@export var combat_framing_enabled: bool = false
...
@export var locked_fov_multiplier: float = 0.78
@export var locked_distance_multiplier: float = 0.62
...
func validate() -> Array[String]:
	...
	if min_pitch_deg < -80.0 or max_pitch_deg > 85.0 or min_pitch_deg >= max_pitch_deg:
...
func _clamp_profile_fields() -> void:
	...
	if not is_finite(height):
		height = 0.55
	height = clampf(height, 0.05, 20.0)
	...
	min_pitch_deg = clampf(min_pitch_deg, -80.0, 80.0)
	max_pitch_deg = clampf(max_pitch_deg, -80.0, 85.0)
	if min_pitch_deg >= max_pitch_deg:
		min_pitch_deg = -55.0
		max_pitch_deg = 70.0

	min_distance = clampf(min_distance, 0.8, 20.0)
	max_distance = clampf(max_distance, 2.0, 40.0)
	if min_distance >= max_distance:
		min_distance = 1.4
		max_distance = 6.5
	if not is_finite(aim_distance) or aim_distance < 4.0:
		aim_distance = 22.0
	else:
		aim_distance = clampf(aim_distance, 4.0, 80.0)
