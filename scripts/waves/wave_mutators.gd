...
static func roll_for_wave(wave: int, run_seed: int) -> Array[StringName]:
	var out: Array[StringName] = []
	if wave < MIN_ROLL_WAVE:
		return out
	var rng := RngService.make_generator(run_seed, RngService.STREAM_WAVES + wave * 7)
...
static func resolve_for_wave(declared: Array, wave: int, run_seed: int, breather: bool, spice: bool) -> Array[StringName]:
	...
	if pool.is_empty():
		if breather:
			return out
		for m in roll_for_wave(wave, run_seed):
			pool.append(m)
		if spice and pool.size() < 2:
			var extra := roll_for_wave(wave + SPICE_WAVE_OFFSET, run_seed)
			for m in extra:
				if m not in pool:
					pool.append(m)
					break
