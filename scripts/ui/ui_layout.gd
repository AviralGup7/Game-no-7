...
static func compute(size: Vector2, text_scale: float) -> Dictionary:
	...
	var reload_button: Rect2 = actions["reload"]
...
static func _action_rects(cluster: Rect2, attack_r: float, minor_r: float) -> Dictionary:
	...
	var reload_button := Rect2(cluster.position, Vector2.ONE * mr * 2.0)
	return {"attack": attack, "dodge": dodge, "swap": swap, "reload": reload_button}
