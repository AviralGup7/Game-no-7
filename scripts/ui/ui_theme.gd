class_name UiTheme
extends RefCounted
## Shared skin + design tokens. Dark-fantasy arena: ember, bronze, parchment.
## Authored chrome lives in res://data/ui/chrome/ (not the locked Kenney pack).

# --- Core palette: ember / bronze / parchment --------------------------------
const INK := Color("070403")          # deepest fire-pit black
const SURFACE := Color("140c08")      # resting leather
const SURFACE_ALT := Color("1e130c")  # raised plate
const SURFACE_TOP := Color("2a1a10")  # light catch
const EDGE := Color("8a5a28")         # bronze rim
const EDGE_SOFT := Color("3d2414")    # inner rim
const TEXT := Color("f3e6d0")         # parchment
const MUTED := Color("b89a78")
const FAINT := Color("6e5340")
const GOLD := Color("e8b44a")         # CTA / rank / currency
const CYAN := Color("e08a45")         # copper ember (info / energy accent)
const HEALTH := Color("e24b3c")       # blood
const STAMINA := Color("e8b44a")      # gold stamina
const XP := Color("c45c2a")           # ember
...
static func icon(name: String) -> Texture2D:
	var chrome := ART + "icon_%s.png" % name
	if ResourceLoader.exists(chrome):
		return load(chrome) as Texture2D
