...
func _start_new_run() -> void:
	...
	_current_run.rng_seed = randi()
	if _current_run.rng_seed == 0:
		_current_run.rng_seed = 1
	if not _daily.is_empty():
		var ds := int(_daily.get("seed", _current_run.rng_seed))
		_current_run.rng_seed = ds if ds != 0 else 1
	...
	EventBus.report_info("Starting run %d mode=%s arena=%s seed=%d%s" % [
		_current_run.run_id, String(_current_run.mode_id), String(arena_id), _current_run.rng_seed,
	...
	if _world_builder.is_valid() and _active_player == null:
		...
		return
	EventBus.run_started.emit(_current_run.run_id, _current_run.rng_seed)
