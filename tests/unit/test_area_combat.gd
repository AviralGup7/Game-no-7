...
## CombatLog ring buffer. Uses parentless Node3D dummies (local == global).
...
static func suite() -> Array:
	...
	var log := CombatLog.new(4)
	for i in range(6):
		log.log_damage(&"sword", "grunt", 10.0, false, false)
	log.log_kill(&"grunt", 10)
	results.append({
		"name": "CombatLog caps at capacity, newest-first recent()",
		"passed": log.size() == 4 and (log.recent(1)[0] as Dictionary)["kind"] == CombatLog.KIND_KILL,
		"why": "size=%d" % log.size(),
	})
	results.append({
		"name": "CombatLog damage_by_source aggregates",
		"passed": absf(float(log.damage_by_source().get("sword", 0.0)) - 30.0) < 0.01,
