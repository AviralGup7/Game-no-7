...
func _test_motion_and_targeting() -> void:
	...
	_check("shooter aim never snaps to a rear threat", targeting.pick_best_target([far, near]) == far)
	near.alive = false
...
func _test_player_scene() -> void:
	...
	_check("attack faces selected enemy", (-player.global_basis.z).dot(Vector3.FORWARD) > 0.99)
	_check("attack plays actual swing", animation._animation.current_animation == "Fire")
	manager.tick(0.01)
	_check("windup cannot hit early", target.hits == 0)
	player.request_attack()
	manager.tick(0.2)
	_check("gun never deals instantaneous melee damage", target.hits == 0)
	...
	_check("early firearm input is buffered", starts[0] == 2 and manager.active_instance().combo_step == 1)
	player._attack_buffer.tick(0.01, player._try_attack)
	...
	_check("dodge follows facing", dodge._dir.dot(Vector3.FORWARD) > 0.99)
	...
	player.request_attack()
	_check("ranged clip selected", animation._animation.current_animation == "Fire")
	...
	animation._physics_process(0.01)
	_check("reload uses supplied animation", animation._animation.current_animation == "Reload")
	player.set_control_enabled(false)
	player.request_attack()
	...
	for weapon_id in equipment.models:
		...
		if equipment._model != null:
			_check("source grip preserved: " + String(weapon_id), equipment._model.position == Vector3.ZERO)
			_check("each gun has an authored muzzle", equipment._model.find_child("Muzzle", true, false) != null)
		_check("all guns use right-hand socket", equipment._socket.bone_name == "handslot.r")
