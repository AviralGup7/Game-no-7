...
# The authored yard moved into ArenaWarehouseYard during the decorator split; the
# decorator still owns the composition that builds it (and the Arena contract).
YARD = "scripts/arena/arena_warehouse_yard.gd"
...
class WarehouseYardTests(unittest.TestCase):
    def test_floor_and_half_match_the_expanded_pit(self):
        ...
        self.assertIn("bounds_half = 32.0", scene)
        self.assertIn("0, 0, -16", scene)
        self.assertIn("16, 0, 0", scene)
        self.assertIn("Wall_N_E", scene)
        self.assertIn("Shape_wall_n_wing", scene)
        self.assertIn("Mesh_yard", scene)
        self.assertIn("size = Vector2(38, 16)", scene)
        self.assertIn("0, 0, 1, 0, 1, 0, -1, 0, 0, -3.3, 1.35, -18.2", scene)
        arena = read("scripts/arena/arena.gd")
        self.assertIn("func get_bounds_half()", arena)
        main = read("scripts/main/main.gd")
        self.assertIn("player.set_bounds(arena.get_bounds_half())", main)
    ...
    def test_default_composition_is_authored_not_scattered(self):
        ...
        self.assertIn("_yard._build_warehouse_compound(", body)
    ...
    def test_warehouse_shell_is_solid_architecture(self):
        yard = read(YARD)
        self.assertIn("func _place_structure(", yard)
        self.assertIn("func _place_column_at(", yard)
        self.assertIn("func _mount_warehouse_model(", yard)
        self.assertIn("res://data/models/warehouse/scene.gltf", yard)
        struct = func_body(YARD, "_place_structure")
        self.assertIn("CollisionLayers.WORLD_BODY_LAYER", struct)
        self.assertIn("_props.add_blocker(", struct)
    ...
    def test_warehouse_model_is_mounted_and_fitted(self):
        compound = func_body(YARD, "_build_warehouse_compound")
        ...
        mount = func_body(YARD, "_mount_warehouse_model")
        ...
        load = func_body(YARD, "_instantiate_warehouse")
        ...
        fit = func_body(YARD, "_fit_warehouse_to_compound")
        self.assertIn("_combined_local_aabb", fit)
        self.assertIn("WAREHOUSE_TARGET", fit)
        self.assertIn("WAREHOUSE_CENTER", fit)
        yard = read(YARD)
        self.assertIn("WAREHOUSE_GATE_WIDTH", compound)
        self.assertRegex(yard, r"WAREHOUSE_CENTER := Vector3\(0\.0, 0\.0, -2[5-9]\.")
        self.assertEqual(yard.count('const MAT_METAL :='), 1)
        self.assertEqual(yard.count('const MAT_BRICK :='), 1)
        self.assertEqual(yard.count('const MAT_WOOD :='), 1)
        gltf = read("data/models/warehouse/scene.gltf")
        self.assertIn('"gate": "open"', gltf)
