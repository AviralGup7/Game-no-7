...
## [Unreleased] — Campaign runtime harness compile fix (2026-09-13)

- Fixed the headless campaign runtime harness (`tests/campaign_runtime/campaign_runtime_inner.gd`,
  added in the "audit work item #7" commit) which had never actually been compiled by the
  engine: the first engine run against a tree containing it (the 2026-09-13 merge to main)
  failed to load it. Five static-analysis repairs against Godot 4.7.2:
  `AABB.volume()` does not exist (`get_volume()`); `DirAccess.file_exists()` is
  instance-only, replaced with static `FileAccess.file_exists()` (the file's other
  `DirAccess` calls are the `*_absolute` static family); `is_equal_approx(Vector3, Vector3)`
  must use the `Vector3.is_equal_approx()` method form; and two `:=` inferences inside the
  transition-cycle coroutine now carry explicit `int` types.
- Recorded a diagnostics caveat: the `diagnostics-4.7.2-*` transcripts committed with the
  "audit work item #7" commit were generated by a workflow run of an *older* tree (its
  backtraces match `run_tests.gd` from before the harness was wired into `run_tests.gd`),
  so that commit's "1663 total, 0 failed" did not cover the harness. The post-merge run on
  main (`GDScript tests: 1664 total, 1 failed`) is the first real engine read of the
  harness, and the failure was confined to this file's parse errors.
